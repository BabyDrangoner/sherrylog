...
void Logger::setFormatter(const std::string & val){
    sherry::LogFormatter::ptr new_val(new sherry::LogFormatter(val));
    if(new_val->isError()){
...
void LogFormatter::init(){
    ...
    for(size_t i = 0;i < m_pattern.size();++i){
        ...
        if(fmt_status == 0){
            ...
            m_error = true;
            vec.push_back(std::make_tuple("<<pattrn_error>>", fmt, 0));
...
sherry::ConfigVar<std::set<LogDefine> >::ptr g_log_defines =
    sherry::Config::Lookup("logs", std::set<LogDefine>(), "logs config");
